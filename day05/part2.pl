...
sub points {
	...
	if( $self->vertical ) {
		...
		# sort so points are always left->right
		my @pts = sort { $a->x <=> $b->x } ( $self->{start}, $self->{end} );
		my $dir = 1;
		$dir = -1 if( $pts[1]->y < $pts[0]->y ); # we're going up not down
		my $y = $pts[0]->y;
		for( my $x=$pts[0]->x; $x<=$pts[1]->x; $x++,$y+=$dir ) {
