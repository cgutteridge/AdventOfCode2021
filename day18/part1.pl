...
REDUCE: while( 1 ) {
	...
	for( my $i=0; $i<@list; ++$i ) {
		...
		if( scalar @r == 5 ) {
			...
			if( $i-1>=0 ) {
				setAtRoute( $data->[0], $list[$i-1]->{r}, $list[$i-1]->{v} + $a );
			}
			if( $i+2<scalar @list ) {
				setAtRoute( $data->[0], $list[$i+2]->{r}, $list[$i+2]->{v} + $b );
			}

			my @pair_route = @r;
			pop @pair_route;
			setAtRoute( $data->[0], \@pair_route, 0 );
	...
	for( my $i=0; $i<@list; ++$i ) {
		if( $list[$i]->{v}>=10 ) {
			# time to split
			setAtRoute( $data->[0], $list[$i]->{r}, [ floor($list[$i]->{v}/2), ceil($list[$i]->{v}/2) ] );		
			next REDUCE;
		}
	}
