...
sub lowest_cost {
	...
	#if( $dep == 20 ) { foreach my $s_i ( @{$plays} ) { print_state( $s_i ); } exit; }
	my $lowest;
	my $lowest_r;
...
#		print $move->[0]."->".$move->[1]." COST ".$move->[2]."\n";
#		print "FROM: $s\n";
#		print "TO  : $new_s\n";
#		print_state( $new_s );	
		my $notes = [$move,$new_s];
		my $cl = lowest_cost( $new_s, $cache, $dep+1, [@$r, $notes] );
		my( $cost, $low_r)  = @$cl;
	...
	$cache->{$s} = [$lowest,$lowest_r];

	return( [$lowest, $lowest_r]);
}
